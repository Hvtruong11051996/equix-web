...
describe('----------19 - Check CHANGE field---------', () => {
    try {
        ...
        it('19 - Check CHANGE field', function (done) {
            (async () => {
                console.log('Start checking CHANGE field')
                ...
                if (Number(marketOverviewWidth.replace(/\D/g, '')) > 540) {
                    ...
                    const row1Text = await row1[2].getText()
                    const row2 = await rows[1].findElements(By.css('.double-cell'))
                    const row2Text = await row2[2].getText()
                    const row3 = await rows[2].findElements(By.css('.double-cell'))
                    const row3Text = await row3[2].getText()
                    const row4 = await rows[3].findElements(By.css('.double-cell'))
                    const row4Text = await row4[2].getText()
                    const row5 = await rows[4].findElements(By.css('.double-cell'))
                    const row5Text = await row5[2].getText()
                    if (/\.\d{3}$/.test(row1Text.replace(/^.*[\n]/, '')) &&
                        /\.\d{3}$/.test(row2Text.replace(/^.*[\n]/, '')) &&
                        /\.\d{3}$/.test(row3Text.replace(/^.*[\n]/, '')) &&
                        /\.\d{3}$/.test(row4Text.replace(/^.*[\n]/, '')) &&
                        /\.\d{3}$/.test(row5Text.replace(/^.*[\n]/, ''))) {
                        console.log('Checked CHANGE field successfully')
                        done()
                    } else {
                        console.log('Checked CHANGE field failed')
                    }
                } else {
