...
describe('----------17 - Check POINT field---------', () => {
    try {
        ...
        it('17 - Check POINT field', function (done) {
            (async () => {
                console.log('Start checking POINT field')
                ...
                if (Number(marketOverviewWidth.replace(/\D/g, '')) > 540) {
                    ...
                    const row1Text = await row1[1].getText()
                    const row2 = await rows[1].findElements(By.css('.double-cell'))
                    const row2Text = await row2[1].getText()
                    const row3 = await rows[2].findElements(By.css('.double-cell'))
                    const row3Text = await row3[1].getText()
                    const row4 = await rows[3].findElements(By.css('.double-cell'))
                    const row4Text = await row4[1].getText()
                    const row5 = await rows[4].findElements(By.css('.double-cell'))
                    const row5Text = await row5[1].getText()
                    if (/\.\d{3}$/.test(row1Text) &&
                        /\.\d{3}$/.test(row2Text) &&
                        /\.\d{3}$/.test(row3Text) &&
                        /\.\d{3}$/.test(row4Text) &&
                        /\.\d{3}$/.test(row5Text)) {
                        console.log('Checked POINT field successfully')
                        done()
                    } else {
                        console.log('Checked POINT field failed')
                    }
                } else {
