...
describe('----------14 - Check CLOSE field---------', () => {
    try {
        ...
        it('14 - Check CLOSE field', function (done) {
            (async () => {
                console.log('Start checking CLOSE field')
                ...
                if (Number(marketOverviewWidth.replace(/\D/g, '')) > 540) {
                    ...
                    const row1Text = await row1[5].getText()
                    const row2 = await rows[1].findElements(By.css('.double-cell'))
                    const row2Text = await row2[5].getText()
                    const row3 = await rows[2].findElements(By.css('.double-cell'))
                    const row3Text = await row3[5].getText()
                    const row4 = await rows[3].findElements(By.css('.double-cell'))
                    const row4Text = await row4[5].getText()
                    const row5 = await rows[4].findElements(By.css('.double-cell'))
                    const row5Text = await row5[5].getText()
                    if (/\.\d{3}$/.test(row1Text) &&
                        /\.\d{3}$/.test(row2Text) &&
                        /\.\d{3}$/.test(row3Text) &&
                        /\.\d{3}$/.test(row4Text) &&
                        /\.\d{3}$/.test(row5Text)) {
                        console.log('Checked CLOSE field successfully')
                        done()
                    } else {
                        console.log('Checked CLOSE field failed')
                    }
                } else {
