...
describe('----------19 - Check click first-page button---------', () => {
    try {
        ...
        it('Check click first-page button', function (done) {
            (async () => {
                console.log('Start checking click first-page button')
                const page = await driver.findElement(By.css('.newsContainer .paginate > div:last-child'))
                const firstPageButton = await page.findElement(By.css('div:nth-child(1)'))
                const firstPageButtonClass = await firstPageButton.getAttribute('class')
                if (firstPageButtonClass === 'disabled') {
