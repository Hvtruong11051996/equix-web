import { truncateText, showTooltip, hideTooltip } from '../helper/func';
import { dragBtn } from '../../SvgIcon/path';
export function size(params) {
    params.gc.font = params.font || '13px Roboto, sans-serif';
    return params.gc.measureText(params.displayValue).width + 16;
}

function toDisplayValue(params) {
    if (typeof params.colDef.formater === 'function') return params.colDef.formater(params)
    return (params.value === 0 || params.value ? params.value : '--')
}

export default function (params) {
    const { gc, config, style, grid, displayValue, colDef, isFirst } = params
    if (params.data && params.data._provider) return
    const b = config.bounds
    function draw(newValue) {
        const display = newValue || displayValue
        gc.fillStyle = params.rowStyle;
        gc.fillRect(b.x, b.y, b.width, b.height);
        if (display === '--') gc.fillStyle = params.rowTextStyle;
        else gc.fillStyle = params.isDown ? style.getPropertyValue('--sell-light') : style.getPropertyValue('--buy-light');
        gc.textBaseline = 'middle';
        let x = b.x + 8 + 0.5;
        gc.textAlign = 'left';
        if (!isFirst) {
            if (colDef.align === 'right' || params.isLast) {
                x = b.x + b.width - 8 + 0.5;
                gc.textAlign = 'right';
            } else if (colDef.align === 'center') {
                x = Math.round(b.width / 2) + b.x + 0.5;
                gc.textAlign = 'center';
            }
        }
        const y = Math.round(b.height / 2) + b.y + 0.5;
        gc.font = params.font;
        const text = truncateText(params, display, b.width - 16);
        if (params.start && display !== '--') {
            const alpha = (300 - params.now + params.start) / 300;
            gc.globalAlpha = alpha < 0 ? 0 : alpha;
            const textWidth = gc.measureText(text).width;
            const textHeight = gc.getTextHeight().height;
            if (gc.textAlign === 'right') gc.fillRect(x - textWidth - 2, y - Math.round(textHeight / 2) - 2, textWidth + 4, textHeight + 4);
            else if (gc.textAlign === 'center') gc.fillRect(x - Math.round(textWidth / 2) - 2, y - Math.round(textHeight / 2) - 2, textWidth + 4, textHeight + 4);
            else gc.fillRect(x - 2, y - Math.round(textHeight / 2) - 2, textWidth + 4, textHeight + 4);
            if (gc.globalAlpha) gc.fillStyle = style.getPropertyValue('--secondary-default') || '#c5cbce';
            gc.globalAlpha = 1;
        }
        gc.fillText(text, x, y);
        gc.closePath();
    }
    if (params.isDown === undefined) params.isDown = false;
    draw();
    return {
        frame: now => {
            params.value = params.data[params.name]
            params.displayValue = toDisplayValue(params)
            params.now = now;
            if (params.value !== params.valueOld) {
                params.isDown = params.value < params.valueOld;
                params.valueOld = params.value;
                params.start = now;
            }
            if (params.start && now - params.start < 320) {
                draw(params.displayValue);
            } else if (params.start) {
                delete params.start
                draw(params.displayValue);
            }
        },
        mouseMove: e => {
            // if (text.length === displayValue.length) return;
            const rect = grid.div.getBoundingClientRect();
            showTooltip(params.displayValue, rect.x + e.detail.gridPoint.x + 10, rect.y + e.detail.gridPoint.y);
        },
        mouseLeave: e => {
            hideTooltip();
        }
    }
}
